...
class FileVideoCapture(object):
    ...
    def __init__(self, path):
        self.path = path
        self.frame = 0
        self.framelist = glob.glob(self.path+'*.png')
        self.currentFrame = None
        self.status = None

    def isOpened(self):
        im = cv2.imread(self.framelist[self.frame])
        return im is not None

    def grab(self):
        self.currentFrame = cv2.imread(self.framelist[self.frame])
        self.status = self.currentFrame is not None
        if self.status:
            self.frame += 1
        return self.status

    def retrieve(self):
        return self.status, self.currentFrame
    ...
    def get(self, argument):
        if argument==cv2.cv.CV_CAP_PROP_FRAME_COUNT:
